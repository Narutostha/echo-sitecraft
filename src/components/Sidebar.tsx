...
const Sidebar = () => {
  ...
  return (
    <motion.nav
      ...
      className="fixed left-0 top-0 h-full w-64 bg-black/95 backdrop-blur-md p-8 z-50"
    >
      ...
      <div className="space-y-8">
        {menuItems.map((item) => (
          <Link
            key={item.label}
            to={item.path}
            className="block text-white hover:text-yellow-400 transition-colors duration-200 text-sm tracking-[0.3em] font-['Eurostile'] uppercase"
          >
            {item.label}
          </Link>
        ))}
